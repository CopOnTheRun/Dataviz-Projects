...
#changing the names to something prettier
        for k,v in self.entities.items():
            mod_frame["entity_id"] = mod_frame["entity_id"].str.replace(k, v, regex = False)
    ...
    def create_entities(ent_dict, case = "power"):
        match case:
            case "power":
                return {f"sensor.{k}_current_consumption" : v for k,v in ent_dict.items()}
            case "contact":
    ...
    @classmethod
    def power(cls):
        power = pd.read_csv("Data/power_usage.csv",)
    ...
    @classmethod
    def contact(cls):
        states = pd.read_csv("Data/fridge_states.csv",)
...
def main():
    ...
    contact["Day of the Week"] = contact["Day of the Week"].apply(lambda x : cd.day_abbr[x])
    ...
    fridge_power = power.mod_frame[power.mod_frame["Device"] == "Fridge"].set_index("Day")

    #joining the fridge power and contact dataframes
    joined = on_states.join(fridge_power, on="Day", rsuffix="_power",)
...
def create_swarm():
    names = {"state_id":"State ID", "entity_id":"Door", "state":"State", 
             "local_updated":"Time", "local_changed": "Changed"}

    on_times = pd.read_csv("Data/fridge_freezer_on.csv",
    ...
    with sns.axes_style("darkgrid"), sns.color_palette("deep"):
        ...
        sns.swarmplot(data = mod_frame, x = "Time", y = "Day", hue = "Door",
                      size = 3.2, alpha = .9, dodge = True, order = list(cd.day_abbr))
        sns.move_legend(ax,"lower right",bbox_to_anchor = (.155,.99),
...
def create_scatter(title, new_index = None, **kwargs):
    ...
    with sns.axes_style("darkgrid"), sns.color_palette("deep"):
        ...
        sns.scatterplot(linewidth=0, alpha=.8, **kwargs)
        for reg,mn,mx in zip(regs,mins,maxs):
            m,b,r,*_= reg
            sns.lineplot(x=(mn,mx),y=(mn*m+b,mx*m+b),
